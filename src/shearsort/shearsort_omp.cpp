...
// Performs parallel Shearsort on the n x n matrix M.
void shearSort(int n, vector<vector<int>>& M) {
    int iterations = ceil(log2(n)) + 1;
    
    for (int iter = 0; iter < iterations; ++iter) {
        // Step 1: Sort rows in an alternating order in parallel.
        ...
        // Step 2: Sort columns in ascending order in parallel.
        #pragma omp parallel for
